...
Peanut Admin `2.0.0` 与 `2.0.1` 建立 fresh-only 正式源码基线。`2.1.0` 在此基础上交付文件、通知、OAuth、支付、会员、任务和导入导出 7 个 Tenant-qualified 官方可选模块。版本使用原生
...
`v2.0.1 -> v2.1.0` 使用显式 transition、配对备份和迁移账本；部署事实由独立
...
- **独立执行**：从 `v2.1.0` 正式 Release 的 Standalone/Multi-tenant 部署、备份、迁移和线上 smoke；它们不属于源码 Release 本身，完成后由 post-deployment 快照证明。
