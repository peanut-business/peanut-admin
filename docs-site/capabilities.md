...
| 文章内容 | **官方 Module 候选正在验收，尚未交付**：既有文章、分类、收藏和多端读取已通过 Tenant-first 资格；固定候选已增加 manifest、权限/菜单目录、管理端 contribution、部署安装与 TenantModule Guard，并通过 PC/UniApp 静态停用合同；仍缺真实数据库、浏览器、`dev` 合入和 Release 身份 | **可选官方模块，否**。不是每个业务应用都需要 CMS | 文件素材、富文本安全、发布状态、客户端展示 | 中，内容安全、跨端能力发现和编辑器升级需要维护 |
