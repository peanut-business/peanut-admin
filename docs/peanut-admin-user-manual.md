...
> 文档口径：Peanut Admin 管理端当前功能。不同账号看到的菜单由角色权限决定。
...
| 财务管理 | 余额明细、充值记录、退款记录 |
| 内容管理 | 文章栏目、文章管理 |
| 应用设置 | 生产准备清单、网站设置、热门搜索、用户设置、支付配置、渠道配置 |
...
- 新密码长度为 12～128 位，两次输入必须一致；修改密码会撤销既有会话，请重新登录验证。
...
- 调账请求必须携带请求级 `Idempotency-Key`；网络超时重试同一请求不会再次变更余额，修改金额或会员后必须生成新的请求。
...
- 当前已验收流程仅支持已支付充值单的首次全额退款；部分/多次退款属于后续扩展，正在实现但数据库资格尚未通过，暂不可用。不要通过重复提交模拟第二笔退款。
...
安装并登录后先打开“生产准备清单”。页面把品牌、通知渠道、文件存储、数据库与文件备份、
任务 Worker、当前管理入口域名/TLS 和管理员账户安全放在同一张表中，并明确每项的作用、
状态、下一动作、责任方和是否阻塞生产。
...
网站设置是安装后品牌信息的唯一管理入口，统一维护管理端、PC 与 UniApp/H5 使用的产品名称、logo、favicon、登录背景、slogan、版权、官网和 GitHub 地址，以及 PC 标题、描述和关键词。保存后各端从同一网站配置读取；无需也不应修改客户端生成文件。

- 图片从素材库选择或上传，按管理端、PC、H5 的字段用途分别检查，不要用同一 favicon 代替全部图片。
- 官网和 GitHub 只接受有效的 `http/https` 地址；仓库默认官网为空，部署后按实际公开地址填写。
- 修改名称、logo、favicon 或版权后，退出并刷新管理端，再检查 PC 与 H5；旧页面缓存未更新时按发布流程刷新，不要直接改数据库或静态产物。
- ICP 等备案信息在同页的备案配置维护，不与产品版权字段混用。
...
填写公众号名称、原始 ID、二维码、AppID、AppSecret 和 Token，并复制回调 URL、业务域名、JS 安全域名和网页授权域名。

- AppSecret 已配置时会掩码回显；保持掩码即可保留原值，不要把掩码当作新凭据。
- 当前服务器回调只支持明文模式；微信平台必须保持明文配置。产品没有 AES 配置入口，不得把兼容/安全模式登记为已支持。
...
- PC 登记回调为当前站点的 `/api/oauth/wechat/redirect/pc`，公众号网页授权登记回调为 `/api/oauth/wechat/redirect/official-account`；两者由服务端固定转到 `/pc/` 与 `/mobile/` 客户端，不能改成根路径 `/oauth/callback`。
- state 和首次补全票据都只有 10 分钟且只能消费一次。补全票据不会出现在客户端 URL；页面关闭或票据过期后重新发起授权。
- 管理端保存成功只证明本地配置有效。真实 AppID/AppSecret、微信后台域名/白名单和一次低风险账号回跳都通过前，不代表生产微信登录可用。
...
- 客服信息只在“装修管理 → 移动端装修 → 客服页面”维护，字段为客服标题、服务时间、联系电话、客服二维码和说明。保存后 H5 客服页再次打开或刷新即读取这五项最新内容。
...
- 旧书签 `/app-setting/decorate` 和 `/app-setting/customer-service` 会转到移动端装修；旧设置接口已经退出，外部集成应改用装修页面接口，不能继续依赖旧入口的保存结果。
...
`multi-tenant` 部署的 PlatformOperator 使用独立平台登录入口管理本实例，不复用普通
管理员会话，也不能读取 Tenant 业务数据。平台侧可创建 Tenant、建立首个 owner、激活或
暂停 Tenant，并按已登记定义开通 TenantModule；模块配置必须通过对应 schema 校验。

Tenant owner/管理员登录后选择自己有权访问的 Tenant。切换 Tenant 会撤销旧上下文并
重新加载当前 Tenant 的角色、菜单和模块；旧 token 不能继续写入。Tenant 被暂停后，新
