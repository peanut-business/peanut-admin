...
当前 inventory 采用不可变 scaffold `v1.1.4` release。该 identity 只属于 scaffold 命名空间，
不是 Peanut Admin 产品 Tag/Release；既有 `v1.1.3` scaffold identity 保持不变。`v1.1.4`
在生产管理端 builder 执行 Vite 前，
