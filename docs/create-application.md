...
当前 inventory 采用不可变 scaffold `v1.1.8` release。该 identity 只属于 scaffold 命名空间，
不是 Peanut Admin 产品 Tag/Release；既有 scaffold identity 均保持不变。`v1.1.8`
继承 `v1.1.7` 的双模式 Runtime、application version 与法律 inventory 合同，并把 app-owned
演示数据命令安装到生产 PHP 镜像。生产管理端 builder 在执行 Vite 前，
...
`DEPLOYMENT_MODE` 选择；PHP 镜像携带应用自己的 `resources/project-resources.json` 和可执行
`scripts/seed-demo-data`，供数据库环境门禁与显式演示初始化使用。登记缺失或与显式部署目标
不一致时仍 fail-closed。
