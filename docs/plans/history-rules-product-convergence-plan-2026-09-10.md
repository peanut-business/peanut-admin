...
候选身份、资格可信性、安全/Tenant/权限、数据或客户ownership、资源越界及最终发布/不可逆操作保留人工Gate。日常主工作目录固定检出 `dev`；功能 worktree 的有效改动须合入主目录实际检出的 `dev`，推送后核对本地工作文件及远端一致。`main` 仅作远端发布分支，日常不推进；仅按具体用户授权通过 PR 推进，不重写共享历史。Application/Core版本和锁对齐推迟到全部开发完成后的发布准备阶段。
规则整理不批准产品重跑或发布；具体恢复条件只从唯一私有状态取得。

## 3. 剩余交付与依赖

环境配置来源继续遵循执行规则 §6.0 `PA-ENV-FILE-ONLY-20260912`，存量范围由问题主登记
`environment_file_configuration.queue` 的 ENV-CONFIG-001..008 保存。恢复中间设计不表示整改已实现，
也不恢复旧候选、运行租约、测试额度或旧 CR03 当前状态。
