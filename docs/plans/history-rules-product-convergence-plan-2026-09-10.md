...
候选身份、资格可信性、安全/Tenant/权限、数据或客户ownership、资源越界及最终发布/不可逆操作保留人工Gate。开发期每次完成并完成最低检查的修改先快进 `dev`，再将 `main` 快进到同一提交；若保护分支已有独立历史，先将该历史合入已验证集成提交，不重写共享历史。Application/Core版本和锁对齐推迟到全部开发完成后的发布准备阶段。
