...
| 阶段 | 目标 | 当前状态 | 主要交付 |
| --- | --- | --- | --- |
| PRE-S01 | 关闭在途核心媒体能力与包事实冲突 | 进行中 | 固定候选、下游采用、Registry 发布 |
| S01 | 冻结 SaaS 业务知识图谱和迁移分类 | 未开始 | 图谱、ADR、表归属账本、迁移合同 |
| S02 | 让 Standalone 运行在单 Tenant 内核上 | 未开始 | 默认 Tenant、身份迁移、业务表租户化 |
| S03 | 完成可信租户上下文和全链路隔离 | 未开始 | Guard、缓存/文件/任务隔离、安全矩阵 |
| S04 | 完成 SaaS Host 租户控制面 | 未开始 | PlatformOperator、Tenant 生命周期、TenantModule |
| S05 | 完成多租户前端和 Host 产品闭环 | 未开始 | 登录选租户、切换、平台端/租户端边界 |
| S06 | 升级、空库、下游和真实浏览器验收 | 未开始 | 双模式安装升级、跨租户对抗、Release 候选 |
| S07 | 发布 SaaS 稳定基线 | 未开始 | Registry 稳定包、应用 Release、文档 |
...
当前首个可领取项仍是 PRE-S01，而不是 S01：

1. 先完成核心媒体通用能力的资格、发布与下游采用；
2. 收口应用和核心在途分支及状态文档；
3. 固定唯一核心消费版本；
4. 然后创建 S01 合同分支，先做图谱和数据归属，不直接写 SaaS Runtime。

任何执行者都不得跳过上述顺序，也不得把独立运营平台代码写入 Peanut Admin/core。
