...
更新时间：2026-08-12 18:50 CST。当前并行阶段为 `MT00-ALPHA5` 与
...
| Alpha.5 发布合同/provenance | 已全绿合入 | Core PR #26/#27/#32/#33；npm artifact `aeeff105df4960db6a70da7ee5597da9a85abdaa`、`packages/web` tree `562043a0a294392a66ea0a305de6554d02646bc8`、72 files、15 exports、SHA-256 `0397260512cb167f3705ddc89d6c03553420339d342ad2f7348ecb52eb7b86a3` |
| Alpha.5 Composer 发布 | 已发布并通过 clean consumer | source annotated tag → `0f3c0a530f2b6369bf5883b2508f40a79501ed98`；split annotated tag、Packagist `peanut-admin/core@0.1.0-alpha.5` → `ef06da45c9e77ae4b194bfc1f859ec007aa0e022`；PHP 8.3.24 clean consumer 精确解析 14 roots |
| Alpha.5 npm 外部身份 | 局部阻塞 | GitHub `npm-release` environment 仅允许 `dev`；exact workflow 已在 Core `dev`；npm CLI 已完成 Web 登录但 trust API 要求账户级 2FA，网页继续要求新增 security key。禁止由代理创建认证凭据；仅阻塞 OIDC binding、npm publish、最终 npm Registry probe/Release |
| MT01 Generator 合同/实现 | 已全绿合入 | Core PR #28/#29/#30；实现 head `880fb0147252b8441f703c120cd5d00ee4678483`，merge `6f24e7ab42e37b56066a3b3be8833a54f087eb3b`；content anchor `30202d73f46c6ab83bf57bd5ce64c24bba9569ec` / tree `d9ca2c39a3b1c3ffa69c26915e5b75732d2f7c35`，683 files，digest `b994a19e…6a96` |
| MT01 Generator integration | fixture 修复 PR 在途 | 首次唯一 Generator Gate 命中 missing-entry fixture 删除自身 runner；最小 Core PR #34 head `2550458fad2391425971f54c4116ef643c0eb8ac` 只改一行。其全绿合入后只重跑失败的 Generator 组；PHP 8.3 static 组尚未运行 |

中断后恢复步骤：

1. 读取两个仓库根 `AGENTS.md`，确认工作目录仍为 `peanut-admin` 和 `peanut-admin-core`。
2. CAP06 已完成；不得重复其真实 MySQL Gate、CAP01–CAP05 或已通过 CI。
3. Core PR #28/#29/#30/#32/#33 已全绿合入；不得重复其 checks。
4. Core PR #34 仅在最新 head 六项检查全部 `COMPLETED/SUCCESS` 后手动合入；随后
   更新 integration branch，只重跑失败的 Generator 组一次，并首次运行 PHP 8.3
   static 组。不得把 PR CI 冒充 MT01 Gate。
5. npm trusted publisher 仍需账户 owner 完成 security-key/账户级 2FA 后绑定 exact
   workflow 与 `npm-release` environment；绑定后唯一调度 workflow，完成 npm clean
   Registry probe、GitHub prerelease 和最终身份记录。Composer 不再重复发布/探测。
