...
| 项目 | 当前值 |
|---|---|
| Candidate | `0600d971818ab2f268797c6bfe04dcbb6415aca7` + 当前未提交上传/装修修改 |
...
|---|---|---|---|---|---|
| ENV-001 | 开发数据库 | 环境门禁与迁移状态 | PASS | 50 migrations；status=current；唯一 root 管理员 | DB 合同、迁移或环境变量变化 |
...
| SETTING-002 | 旧页面装修/客服设置 | 保存后由当前客户端消费 | FAIL | 与 Decoration Runtime 双入口错配，已创建独立修复任务 | 双入口收敛修复完成 |
...
| ADMIN-ALL | 管理后台全部菜单 | 逐菜单真实新增/编辑/删除或保存 | IN-PROGRESS | 本轮执行中 | 按子项结果决定 |
