...
| 项目 | 当前事实 | 对闭环的含义 |
|---|---|---|
| Application | `origin/dev@9f33eb9fac44858d5d0575e1a760eec0d3af0694` | PC00—PC31 的既定顺序切片已合入；PC32 是下一关键路径 |
...
| 运维 | Platform 已采用 Core Ops 状态/任务合同，提供有界诊断包与应用备份中心 | 恢复和维护写入仍未开放 |
| 备份 | 生产登记有 DB + `php-storage` 配对门禁；应用已有单一受信 Provider、schema 1 manifest、任务/证据账本、受信 worker 和 Platform 备份中心 | 真实配对制品及恢复验证仍由 PC32 完成，不把合同 smoke 冒充可恢复证据 |
...
|---|---:|---|
| 队列任务 | 19 | PC00—PC70 |
| 已完成 | 10 | PC00、PC01、PC02、PC10、PC11、PC12、PC20、PC21、PC30、PC31 |
| 进行中 | 0 | — |
| 部分完成 | 0 | — |
| 外部阻塞 | 0 | — |
| 未开始 | 9 | PC32—PC70 |
| 当前关键路径 | PC32 | 恢复到登记新目标并验证；尚未领取 |
...
| B 可安装、可诊断 | 已完成 | PC10—PC21 已合入；CLI、guided/automatic installer、Web 向导、首次运行清单、只读 Ops Console 和脱敏诊断包存在 | 正式数据库/浏览器/released-scaffold 组合资格归 PC70 | 保持 schema 与权限边界，继续 PC32 |
| C 可备份、可恢复 | 部分完成 | PC30 Provider/manifest 合同；PC31 Core 任务采用、备份中心、evidence、受信 worker、心跳和 fencing | 真实配对制品、隔离恢复和代表验证 | PC32 恢复到新目标并验证 |
...
| PC30 | 已完成 | Runtime `2380593b51680de48962eb360514530bdb356fe5`；`dev@dd6a877d1d31a583d0221c0a7b6a3ca325da8e77` / PR #287 | 单一受信 DB/文件 Provider、逻辑新目标、一次尝试和 schema 1 manifest 已形成；合同固定容量、停写窗口、资源/镜像身份、配对 artifact、SHA-256、保留与清理责任，拒绝路径/命令/凭据输入 | 真实配对制品/恢复和正式组合资格归 PC32/PC70；不阻塞 PC30 合同完成 | 保持 Provider/manifest 唯一合同 |
| PC31 | 已完成 | Runtime `994f8e7961af9be8834bc9c362f268546eeff6f4`；`dev@9f33eb9fac44858d5d0575e1a760eec0d3af0694` / PR #289 | Core 任务、权限、幂等、并发拒绝和原子审计已由 Application 采用；受信 worker 具 60 秒心跳/revision fencing，Platform 展示最近任务、失败码和最新已验证 evidence，Web 不接收或返回 handler/path/command/credential | 真实配对 artifact、新目标恢复和浏览器/released-scaffold 资格归 PC32/PC70；不阻塞 PC31 实现完成 | PC32 绑定登记的新隔离目标并验证恢复 |
| PC32 | 未开始 | — | — | 无；PC31 已满足 | 恢复到新目标并验证 |
