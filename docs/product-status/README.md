...
| `PA-UPGRADE-002` | 应用升级就绪与目标兼容检查 | 已实现，待验收 | PC41 已由 PR #307 合入 dev：Platform 只读升级就绪固定 source/target Release、资格身份、migration、目标 Module 源码与 Kernel、zero-write scaffold ownership/conflict、配对备份/恢复 evidence、planned-upgrade 维护窗口和 recovery pointer；目标只来自固定受信目录，HTTP 不能选择路径、URL、命令、Release、镜像或凭据。实际升级执行归 PC42，正式 released-scaffold 组合资格归 PC70。 |
