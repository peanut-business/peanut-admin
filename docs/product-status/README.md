...
| `PA-BACKUP-001` | 受信配对备份 Provider 与制品合同 | 已实现，待验收 | PC30 已由 PR #287 合入 dev：应用直接采用 Core Ops Provider Registry，固定单一 DB/文件 Provider、逻辑新目标、一次尝试和 schema 1 manifest；容量、停写窗口、稳定资源/镜像身份、固定配对 artifact、SHA-256 及责任边界已形成。任务提交、真实备份/恢复和正式组合资格分别归 PC31、PC32、PC70。 |
