...
| `pa_refund_record/log` | 同一来源订单可有多笔退款记录，每笔记录包含逐次渠道尝试 | 失败重试只追加尝试，不再扣余额或累计充值 |
...
- 每笔退款锁充值订单并确认已支付，在锁内汇总历史退款、校验剩余可退金额；余额扣减、101 流水、订单 refund 状态和独立退款记录同事务提交。请求还受 `Idempotency-Key` 约束。
...
3. 支付订单行锁和 paid guard 位于入账前，`uk_transaction_id` 存在；部分退款在订单锁内校验累计金额，退款重试不扣第二次；后台调账与创建退款均有请求级幂等记录。
