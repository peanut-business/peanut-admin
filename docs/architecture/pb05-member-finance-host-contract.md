...
> 状态：Accepted（历史单次全额退款基线）；部分/多次退款扩展：实现中（数据库资格阻塞，暂后置）
>
...
| `pa_member.total_recharge_amount` | 成功充值累计额；每笔充值退款同步扣回 | 不是当前余额，不受后台普通调账影响 |
...
| `pa_refund_record/log` | 已验收基线为同一来源订单一条主退款记录及逐次渠道尝试 | 部分/多次扩展目标为同一订单多笔记录；当前尚未通过数据库资格，失败重试只追加尝试 |
...
- 历史首次退款锁充值订单并确认已支付；余额扣减、101 流水、订单 refund 状态和主退款记录同事务提交。失败重试只复用该记录，不再次扣款。
- 部分/多次退款扩展目标是在订单锁内汇总历史退款并建立独立记录；2026-08-20 `audit20b` 的 `30+70` 数据库资格在第二笔失败，当前不作为 Accepted 能力。
...
禁止修改核心仓、`vendor/`、`node_modules/`、既有产品 schema/seed、路由/菜单、会员/财务页面、支付/OAuth/通知、PB08A 品牌输入或 SaaS 设计；本次幂等表仅通过追加迁移落地。
...
3. 支付订单行锁和 paid guard 位于入账前，`uk_transaction_id` 存在；封存 F02 证明首次退款重试不扣第二次；后台调账与创建退款均有请求级幂等记录。部分/多次退款的 `30+70` 数据库资格尚未通过。
...
通过只表示应用会员/财务产品 owner、唯一余额写入链、事务/防重边界和测试 owner 已固定；它不表示部分/多次退款扩展已通过数据库资格。它不批准核心 R01/R02 或 Tenant membership 消费，不实现新支付渠道、订单系统、多币种/冻结资金/会计总账，不开始 PB06、PB07 或 SaaS。
