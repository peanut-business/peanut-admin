...
| PB06 | 内容与装修 | 应用 Module 唯一 Runtime；核心无产品域 owner | 资源 provenance、分类/发布规则和共享读取 DTO 合同已完成 | 不复制三端业务状态机，不迁产品模型到核心 |
...
PB04 从网站基础设置开始；该首片已按本节更新后的应用 owner 路线完成。权限 Host、管理员/RBAC CRUD、字典、文件素材、任务/导入导出与日志/维护随后均按独立合同完成，PB04 系统基础域现已关闭。
