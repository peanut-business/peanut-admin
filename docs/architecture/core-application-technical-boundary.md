...
> 状态：planned；本页冻结存储驱动提取的边界与后续执行规则，不声称本轮 Runtime 已完成迁移。
>
...
核心拥有产品无关机制及明示技术状态。应用拥有业务规则、数据、UI 和宿主装配。每张表、每个状态机和每项业务规则只有一个 owner；共享机制不因此取得产品数据或产品状态的所有权。

## 2. 五域职责原则

| 域 | Core | 应用 | 本轮状态 |
| --- | --- | --- | --- |
| Settings | typed、secret、scope 等产品无关能力 | key/schema/default、业务设置和 ThinkPHP 宿主存储 | 只冻结复用规则，不重建实现 |
| Storage | `StorageDriver`、对象 key 规则和低层文件传输机制 | Provider SDK 装配、凭据解密、用途、授权、对象账本、补偿和产品生命周期 | 本轮唯一执行域 |
...
应用装配事实位于 `server/app/AppService.php` 与 `server/app/common/service/storage/`：
`StorageDriverFactory`、`ObservedStorageDriver`、`StoragePath`、`StorageRepository`、
`QiniuStorageHttpTransport` 以及应用现有配置/凭据组合根。应用侧旧的
`StorageDriver.php` 与 `driver/{Local,Aliyun,Qcloud,Qiniu}StorageDriver.php` 已删除，
应用 `FileMediaHostTest.php` 已按新边界修正。`/opt/homebrew/bin/php -l` 对受改 PHP 文件通过，
`cd server && /opt/homebrew/bin/php tests/Productization/FileMediaHostTest.php` 返回
`PB04-FILE-MEDIA-HOST-001 passed`；Tenant 资格测试因 worktree 无 vendor 且未安装/复用而未运行。
Composer/lock 未改，正式消费仍等待 Core immutable split 包身份。
