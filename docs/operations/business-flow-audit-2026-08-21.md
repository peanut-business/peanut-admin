...
本报告中的“待发布”结论是 2026-08-21 的截面，不代表当前状态。随后固定 `main` 候选
`df6e018ffecabba37b0bbdf44b79067da7341552` 完成 L2 P0-E 7/7，发布 `v3.0.5`，并更新登记的
Standalone（`peanut-admin.007345.xyz`）与 Multi-tenant（`pa-*.007345.xyz`）Demo。两个目标
均运行 metadata `3.0.5`、健康检查通过；Platform、Standalone Admin、Tenant A 的真实 HTTPS
登录均成功，公开 Demo 密码摘要已统一为 `peanut1234`。发布后发现的 `deploy-release` 更新模式
缺陷已通过 PR #214 合入 `dev`、PR #215 合入 `main`，不改变已发布 Tag。
