...
| Project bootstrap and execution rules | `AGENT_EXECUTION_RULES.md`; rule metadata in `docs/document-registry.json` | `AGENTS.md` only routes the minimum inputs |
| Current execution authority, control writer, failure budget and recovery | The single private control state resolved by execution policy §0.2; immutable evidence references | Public navigation and plans do not copy these fields; live resource holder stays in the existing lease store |
...
| Product-closure, consumer-ready delivery and post-release enhancement ownership, package compatibility and evidence-retention decisions | accepted ownership decision, package manifests/locks, fixed commits, PRs, completed minimum verification and Git-traceable cleanup baseline | `docs/architecture/product-closure-ownership-and-adoption.md`, `docs/architecture/product-closure-core-application-compatibility.md`, `docs/product-status/product-closure-observability.md`, `docs/plans/consumer-ready-product-plan.md`, `docs/plans/post-release-enhancement-plan.md` and `docs/maintenance/consumer-ready-evidence-retention.md`; stable capability completion still returns to the capability ledger |
...
| Target Application routing, source organization and cross-Module call rules | `docs/architecture/application-module-blueprint/coding-standards.md` and the accepted blueprint decisions | current Composer, Application routes/providers, Module manifests and source paths are implementation evidence only; target status is not Runtime completion evidence |
