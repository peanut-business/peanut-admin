...
> Release metadata 与 CompanyOS 资源登记为准。

## 事实矩阵

| 范围 | 原值或失真表述 | 当前事实与证据 | 处理 |
| --- | --- | --- | --- |
| 开发数据库 | 旧 3306 端口与 `peanut_admin` 库被称为权威数据库 | CompanyOS `resources/development-infrastructure.yaml` 登记 `peanut-admin-mysql84-development`：development、MySQL 8.4.10、`192.168.192.2:20183/peanut_admin_development`、状态 `provisioned` | 更新 parity 报告和根事实源；旧值只允许出现在本登记及 `output/` 历史证据 |
...
| 本地安装示例 | 公共指南使用 `127.0.0.1`、`localhost` 和示例库 `peanut_admin` | 这些是外部克隆的中性自有环境示例，不是 CompanyOS allocation；公司内部日常开发只使用根 `AGENTS.md` 登记资源 | 保留中性模板；禁止把退出的公司数据库地址写回当前指引 |
