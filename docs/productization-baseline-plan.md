...
| PB04 | 系统基础域收口 | 待开始 | 认证/权限、设置、文件、字典、任务、日志/维护按批准边界形成唯一实现；核心候选未经授权不替换应用 |
...
网站设置首片的真实存储表是 `pa_config`，不是 `pa_system_config`。核心现有 `PdoSettingRepository` 使用 `pa_setting_*` 且没有可替换存储端口；在获批的核心 Host 合同或明确的 schema 前滚合同形成前，不修改核心 Runtime、不双写两套表，PB04 保持未开始。
