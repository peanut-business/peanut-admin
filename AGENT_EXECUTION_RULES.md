...
- 阶段0–4、T01、CQ、C01、CR01及CR02开发交付已验收，不重新派发阅读或已验收修复；当前消费优先路线及CR03实际任务书见方案§12.6–12.8，CR03仍须固定候选与独立资格，CR04负责交付。具体支持范围和缺口由问题主登记维护；后续子智能体最多20且服从实际容量。
- 已批准消费路线内，每个阶段由根任务回收前置、填实完整任务书及技术 Gate 后交独立任务，不重复索取同范围授权。必要补正沿用原任务/ID；新的业务范围和保留的高风险人工 Gate 仍由用户裁定。
- 历史只用于有效决定和未闭环问题取证；现行规则/方案直接原位修正，失效配置和重复说明直接移除，不新增“关闭”副本或备份报告。保留原始证据与唯一任务结果，必要时定向补证。
- 根任务 `01a07fd6-296d-7cf2-8434-77c57925fc14` 持有总调度与正式登记；不创建Goal或启用Goal自动续作。已结束任务不得因旧 START 或回报自动恢复。
- 不创建或恢复周期监督、十分钟确认唤醒或其他定时器。Goal续作、旧消息、回报、重启及压缩不是独立授权；续作先核现行检查点及已批准范围。
- 技术前置或用户保留的人工 Gate 未满足时只阻塞其依赖项；阶段5按批准批次修复，独立资格与交付不能由开发完成替代。本节优先于通用吞吐条款。
