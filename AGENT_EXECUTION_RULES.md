...
- 完整资格 Gate 只用于固定候选验收，不得作为迭代调试入口。某组失败后，必须先在 Development mode 使用同类已登记真实资源完成能够复现该失败的最小聚焦验证；聚焦验证未通过时，禁止 reseal、更新候选身份或启动新的完整资格。
- 同一子系统连续两次因相邻机制失败时，停止点状症状修复；先明确该抽象边界的输入类型、运行模式、成功/失败/恢复状态和资源副作用矩阵，再完成一次边界级修复与聚焦验证。
...
- 每个去重后的失败原因只允许一次定向诊断、一次修复和一次聚焦重跑；同组重跑若暴露
  不同的相邻机制，必须按第 4 节先完成边界矩阵和聚焦验证，不得继续点状修补。已经通过
