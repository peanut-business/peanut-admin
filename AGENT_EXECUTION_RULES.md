...
- 同一仓库、同一用户目标和同一验收范围内，先在本地功能分支上批量完成实现、合同、文档和必要修复，再运行一次最低充分验证，最后一次推送功能分支并创建一个 PR。不得为每个小修改、每次本地提交或每个验证组重复 push 和创建 PR。
- 本地提交可按可回滚的逻辑分块保留，但在远程交付前应合并或压缩为一个可审查的任务候选；分支、PR 和 Gate 的数量不得成为交付目标。
- 只有在不同仓库、不同文件 owner、不同资源所有者，或存在独立安全/合同停止线时才拆分为多条交付线；拆分时必须明确下游关系和一次性验收计划。
- 远程 `dev`/其他受保护分支禁止直接 push 时，不要先进行会被拒绝的直接推送；在本地候选冻结后直接推送一次功能分支，通过一个 PR 完成合入。本地合并只用于最终候选预测和验证，不替代受保护分支的远程 PR 流程。
