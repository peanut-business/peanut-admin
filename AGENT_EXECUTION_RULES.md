...
本节只适用于 `pa-convergence-plan-20260910`，控制修订 `PA-USER-STEP-GATES-20260910`。
每次续作、消息插入或压缩后先读 `docs/plans/history-rules-product-convergence-plan-2026-09-10.md` 与问题主登记的当前批次；根任务另读取原本机未跟踪检查点的 `step_approval`、owner、写集和恢复点。该私有控制文件不是公开仓库文档或普通 clone 前置。

- 阶段1已有材料归并及必要补正、阶段2决定/经验/知识收敛已完成；不重新派发阅读，后续为阶段3查现状/补遗漏/排修复、4防复发规则、5分批实际修复、6独立验收交付，仍须另行确认。29项不是范围上限；具体批准支持范围、决定、知识和当前能力应有去向，缺源不冒称已吸收。后续子智能体最多20且服从实际容量。
- 阶段3任务书已获用户明确批准，按独立任务执行。每个新阶段先由根任务回收前置、提出完整任务书；用户显式确认或按方案§7.1满十分钟无反对的预授权执行。未决选择及须单独核对授权的高风险操作不适用超时确认。必要补正沿用原任务/ID，不重新派发已验收内容。
- 历史只用于有效决定和未闭环问题取证；现行规则/方案直接原位修正，失效配置和重复说明直接移除，不新增“关闭”副本或备份报告。保留原始证据与唯一任务结果，必要时定向补证。
- 根任务 `01a07fd6-296d-7cf2-8434-77c57925fc14` 持有唯一Goal/正式登记，不启用Goal自动续作；旧侧栏实施任务保持撤销。
- 不恢复旧周期监督。最新用户授权仅允许根任务在完整下一任务书送达后设置一次十分钟确认唤醒；检查点绑定期限/方案SHA/消息和唯一消费状态。反对、暂停或方案变更使窗口失效。Goal续作、旧消息、回报、重启及压缩不是独立授权。
- 等待确认是正常状态；本节优先于通用吞吐/并行条款。阶段1–3不改Runtime/业务测试；阶段4落实规则，阶段5按批准批次修复，阶段6单独验收。阶段3须给出可执行批次、依赖及有依据的时间估算，不只写问题报告。
- 模型与推理分别选最低充分档位，不默认继承最高档。未知最后操作先只读核验，规则冲突仅恢复控制，不重派。
- 2026-09-11最新授权为方案§12.6消费优先路线：C01已交付，先修消费/升级阻塞，协调发行后做独立资格与交付；内部C02–C12非阻塞项后置。用户将推进方式交根安排，根核前置并填实每阶段实际写集/资源/候选后，按新独立任务连续衔接，不逐小步或重复同范围确认。安全/数据/厂商有效功能、不可变身份和测试门禁不减；不以internal标签或兼容桥解决客户升级。新的业务范围/破坏性合同/客户生产数据及未知副作用仍须用户裁定，不创建Goal/定时器。
- 文件约束不是所有工具硬拦截或压缩Hook已验证的证明；未覆盖范围和实际限制如实登记。
