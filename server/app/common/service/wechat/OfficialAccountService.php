 ...
 * 微信公众号能力边界。
 *
 * 菜单发布使用可注入传输器，验收无需真实公众号；生产默认使用 cURL。
 * 回调只处理参考系统实际可证明的明文消息，不宣称 AES 配置已生效。
 */
class OfficialAccountService
{
    ...
    public function publishMenu(string $appId, string $appSecret, array $menu): void
    {
        ...
        $tokenUrl = self::TOKEN_URL . '?' . http_build_query([
        ...
        $body = json_encode(
            ['button' => $this->wechatButtons($menu)],
            JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR
        );
        $result = $this->requestJson(
            'POST',
            self::MENU_URL . '?access_token=' . rawurlencode($accessToken),
            $body
        );
    ...
    private function request(string $method, string $url, string $body): array
    {
        if ($this->transport !== null) {
            $result = ($this->transport)(
                $method,
                $url,
                ['Accept: application/json', 'Content-Type: application/json', 'User-Agent: PeanutAdmin/1.0'],
                $body
            );
        ...
        return array_map(function (array $item): array {
            ...
            if ($children !== []) {
                return [
                    ...
                    'sub_button' => $this->wechatButtons($children),
                ];
            }
