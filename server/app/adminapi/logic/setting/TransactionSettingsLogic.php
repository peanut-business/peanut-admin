...
class TransactionSettingsLogic extends BaseLogic
{
    ...
    {
        $setting = TransactionSettingTenantRepository::settings($context)->findOrEmpty();
        if ($setting->isEmpty()) {
    ...
    public static function setConfig(TenantContext $context, array $params): void
    {
        unset($params['tenant_id']);
        $current = self::getConfig($context);
        TransactionSettingTenantRepository::update($context, [
