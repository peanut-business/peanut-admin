...
Git delivery follows execution rules §5.1: use local `feat/<description>` branches, merge completed and minimally checked work into local `dev`, then push only `dev`. Feature branches are pushed only for an explicitly needed remote collaboration, backup or review. Ordinary development does not authorize advancing `main`, tagging or publishing; `dev → main` requires release preparation, applicable qualification and human approval through a PR. Defer Application/Core version and lock alignment until all development work is complete and release preparation begins. Preserve shared history and fixed-candidate, signature, security/data/resource and release Gates; clean only merged, unowned task branches/worktrees.
