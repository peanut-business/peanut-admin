...
- 先按影响确定阻塞范围，不把单个失败扩大为全项目停工：安全、租户隔离、授权、数据损坏或核心业务断言失败，阻塞受影响实现及合入；Schema/API 合同、不可变依赖身份失败，阻塞受影响采用、合入和发布；格式、文档、依赖元数据及机械 CI 失败，只阻塞当前 PR 的合入或发布收口。
- “允许继续”不等于通过或可发布。失败候选留在功能分支或 stacked fix 分支；红色检查不得合入 `dev`/`main`，阶段完成声明必须等待该阶段 required Gate 全部关闭。
- 与失败产物无依赖、文件 owner 不冲突的实现、只读审计、合同/fixture 准备可以并行。等待 CI 时保持 1 条关键路径和最多 2 条真正有用的独立准备线；没有可并行项时说明具体依赖。
- 先运行覆盖改动的最低本地 Gate；依赖锁或 manifest 变更必须包含与 CI 一致的严格校验及 lock 精确身份检查。失败按原因去重，只重跑失败组，不重复已通过组或扩大套件。
- 状态更新只在 Gate 开始、状态变化、异常、超时或完成时发送，明确仓库和 PR；子任务只回传结论、证据和阻塞，完成后立即收敛，避免轮询日志和重复上下文消耗。
