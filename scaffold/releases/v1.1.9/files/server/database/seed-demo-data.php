...
function demoPlan(): array
{
    return [
        ...
        'articles' => [
            [
                'category' => '产品介绍',
                'title' => 'Scaffold Product Token 产品简介',
                'desc' => '一套可直接演示的管理端、PC 与移动端应用基线。',
                'abstract' => 'Scaffold Product Token Demo content',
                'content' => '<p>Scaffold Product Token 提供管理端、PC 与移动端共用的内容和配置能力。</p><p>这些内容属于演示实例，可在文章管理中编辑或删除。</p>',
                'click_virtual' => 128,
                'sort' => 90,
            ],
            [
            ...
            [
                ...
                'abstract' => 'Scaffold Product Token Demo content',
                'content' => '<p>当前演示环境运行 Scaffold Product Token v1.1.5，已完成 50 到 54 条迁移及最低线上 smoke。</p>',
                'click_virtual' => 64,
                'sort' => 50,
            ],
            [
...
try {
    ...
    $insertArticle = $pdo->prepare(
        ...
        . 'VALUES (:tenant_id, :cid, :title, :description, :abstract, \'\', \'Scaffold Product Token Demo\', :content, :click_virtual, 0, :sort, 1, :create_time, :update_time)'
    );
    ...
    foreach ($plan['articles'] as $article) {
        $findArticle->execute(['tenant_id' => $tenantId, 'title' => $article['title'], 'author' => 'Scaffold Product Token Demo']);
    ...
    foreach ($plan['tags'] as $name) {
        $tag->execute([
            'tenant_id' => $tenantId,
            'name' => $name,
            'remark' => 'Scaffold Product Token Demo synthetic data',
            'create_time' => $now,
            'update_time' => $now,
        ]);
